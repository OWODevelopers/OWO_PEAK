...
#pragma warning restore CS0109
        ...
        internal static class CharacterMovement_Patch
        {
            ...
            [HarmonyPatch(nameof(CharacterMovement.AddGroundSample))]
            [HarmonyPostfix]
            private static void Landing(CharacterMovement __instance, CharacterMovement.PlayerGroundSample sample)
            {
                Character character = Traverse.Create(__instance).Field("character").GetValue<Character>();

                if (character != null && character.IsLocal)
                {
                    if (!character.data.isGrounded) //TODO only check once
                    {
